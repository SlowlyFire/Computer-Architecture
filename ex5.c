			...
			//	instead of kernelApply function
				...
				for(ii = max(i-1, 0); ii <= min(i+1, m-1); ii++) {
					...
					for(jj = max(j-1, 0); jj <= min(j+1, m-1); jj++) {
						...
						sum.red += ((int) backupOrg[resultSrc].red) * resultKernel;
						sum.green += ((int) backupOrg[resultSrc].green) * resultKernel;
						sum.blue += ((int) backupOrg[resultSrc].blue) * resultKernel;
					}
				}
